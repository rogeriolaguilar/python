...
def select_columns(my_table, col_indices):
    ...
    """
    col_selector = lambda row: [ row[index] for index in col_indices ]
    table = list(map(col_selector, my_table ))
...
def sort_by_column(my_table, col_idx):
    ...
    """
    my_table.sort(key = lambda list: list[col_idx], reverse=True)
    pass
    
#table = [[1,2,3],[3,5,2],[6,3,1],[3,6,1]]
